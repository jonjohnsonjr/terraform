...
func (g *Graph) Walk(ctx context.Context, walker GraphWalker) tfdiags.Diagnostics {
	ctx, span := otel.Tracer("github.com/hashicorp/terraform").Start(ctx, "Graph.Walk")
	defer span.End()
...
func (g *Graph) walk(ctx context.Context, walker GraphWalker) tfdiags.Diagnostics {
	...
	// Walk the graph.
	walkFn := func(ctx context.Context, v dag.Vertex) (diags tfdiags.Diagnostics) {
		...
		if pn, ok := v.(GraphNodeModuleInstance); ok {
			vertexCtx = walker.EnterPath(ctx, pn.Path())
