...
func (n *nodeExpandOutput) DynamicExpand(ctx context.Context, ectx EvalContext) (*Graph, error) {
	// ctx, span := otel.Tracer("github.com/hashicorp/terraform").Start(ctx, "nodeExpandOutput.DynamicExpand")
	// defer span.End()
...
// GraphNodeExecutable
func (n *NodeApplyableOutput) Execute(_ context.Context, ectx EvalContext, op walkOperation) (diags tfdiags.Diagnostics) {
	state := ectx.State()
	if state == nil {
		return
	}
...
// GraphNodeExecutable
func (n *NodeDestroyableOutput) Execute(_ context.Context, ectx EvalContext, op walkOperation) tfdiags.Diagnostics {
	state := ectx.State()
	if state == nil {
		return nil
	}
