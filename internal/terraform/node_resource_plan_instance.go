...
// GraphNodeEvalable
func (n *NodePlannableResourceInstance) Execute(ctx context.Context, ectx EvalContext, op walkOperation) tfdiags.Diagnostics {
...
func (n *NodePlannableResourceInstance) dataResourceExecute(ectx EvalContext) (diags tfdiags.Diagnostics) {
	config := n.Config
	addr := n.ResourceInstanceAddr()
	...
	change, state, repeatData, planDiags := n.planDataSource(ectx, checkRuleSeverity, n.skipPlanChanges)
	...
	// working state
	diags = diags.Append(n.writeResourceInstanceState(ectx, state, refreshState))
	if diags.HasErrors() {
		return diags
	}
...
func (n *NodePlannableResourceInstance) managedResourceExecute(ctx context.Context, ectx EvalContext) (diags tfdiags.Diagnostics) {
	config := n.Config
	addr := n.ResourceInstanceAddr()
	...
	if importing {
		instanceRefreshState, diags = n.importState(ctx, ectx, addr, provider, providerSchema)
	} else {
		var readDiags tfdiags.Diagnostics
		instanceRefreshState, readDiags = n.readResourceInstanceState(ectx, addr)
	...
	// refresh step below.
	diags = diags.Append(n.writeResourceInstanceState(ectx, instanceRefreshState, prevRunState))
	if diags.HasErrors() {
		return diags
	}
	// Also the refreshState, because that should still reflect schema upgrades
	// even if it doesn't reflect upstream changes.
	diags = diags.Append(n.writeResourceInstanceState(ectx, instanceRefreshState, refreshState))
	if diags.HasErrors() {
		return diags
	}
	...
	if !n.skipRefresh && !importing {
		s, refreshDiags := n.refresh(ctx, ectx, states.NotDeposed, instanceRefreshState)
	...
	if !n.skipPlanChanges {
		...
		diags = diags.Append(n.replaceTriggered(ectx, repData))
		if diags.HasErrors() {
			return diags
		}

		change, instancePlanState, repeatData, planDiags := n.plan(
			ectx, nil, instanceRefreshState, n.ForceCreateBeforeDestroy, n.forceReplace,
		)
		diags = diags.Append(planDiags)
		if diags.HasErrors() {
		...
		diags = diags.Append(n.writeResourceInstanceState(ectx, instancePlanState, workingState))
		if diags.HasErrors() {
			return diags
		}
		...
		diags = diags.Append(n.writeResourceInstanceState(ectx, instanceRefreshState, workingState))
		if diags.HasErrors() {
			return diags
		}
...
func (n *NodePlannableResourceInstance) replaceTriggered(ectx EvalContext, repData instances.RepetitionData) tfdiags.Diagnostics {
	...
	for _, expr := range n.Config.TriggersReplacement {
		ref, replace, evalDiags := ectx.EvaluateReplaceTriggeredBy(expr, repData)
...
func (n *NodePlannableResourceInstance) importState(ctx context.Context, ectx EvalContext, addr addrs.AbsResourceInstance, provider providers.Interface, providerSchema *ProviderSchema) (*states.ResourceInstanceObject, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	absAddr := addr.Resource.Absolute(ectx.Path())
	...
	instanceRefreshState, refreshDiags := riNode.refresh(ctx, ectx, states.NotDeposed, importedState)
	diags = diags.Append(refreshDiags)
	if diags.HasErrors() {
	...
	diags = diags.Append(riNode.writeResourceInstanceState(ectx, instanceRefreshState, refreshState))
	return instanceRefreshState, diags
}
